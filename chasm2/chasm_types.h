...
// #define token (def) added at end of enum for now.
// Update when adding ( and ) as tokens to implement #define psh(r0) type macros
// The fsms use toks_t values as indices into arrays.
// def would cause a problem, but ldr r0, #define is unlikely code.
enum toks_t { 
...
struct toki_t {
    ...
    char *filename;                 // name of file with line
