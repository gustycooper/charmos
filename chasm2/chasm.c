...
int errors() {
    ...
    for (int i = 0; i < linenum-1; i++) {
        if (toks[i].linetype > comment && toks[i].linetype != number && toks[i].linetype != ident && toks[i].linetype != def) {
            fprintf(stderr, "+++ Error +++ Filename: %s, Line number: %d\n", toks[i].filename, toks[i].linenum);
            for (int j = 0; j < toks[i].numtoks-1; j++) // numtoks-1 to ignore endd token
...
void dodefines() {
    ...
    for (int i = 0; i < linenum-1; i++) {
        ...
        // No use iterating through tokens if the input does not contain a #define statement
        if (havedefine && toks[i].linetype != comment && toks[i].linetype != def) {
            for (int j = 0; j < toks[i].numtoks-1; j++) {
                if (toks[i].toks[j].toktype == ident) {
                    struct defdictval ddv;
...
void process_file(char *fn) {
    ...
    while (1) {
        ...
        if (buf[0] == '#' && buf[1] == 'i' && buf[2] == 'n' && buf[3] == 'c' && buf[4] == 'l' &&
            ...
            linenum++;               // counts total linenums (all files)
            filelinenum++;           // counts linenums in current file
...
int main(int argc, char **argv) {
    ...
    close(fd);
    // Loop - reads line, tokenizes line in tok, saves tok in array toks
    process_file(input); // process input file and any #include files
